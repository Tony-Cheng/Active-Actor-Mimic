...
def standard_optimization_no_mem(policy_net, target_net, samples, optimizer, gamma, lambda1, lambda2, batch_size, device):
    (states, actions, rewards, next_states, dones, rollout_rewards, rollout_offsets,
...
def demo_optimization_epochs(policy_net, target_net, memory, optimizer, gamma, lambda1, lambda2, batch_size, epochs, device):
    total_batch_size = epochs * batch_size
    bs, ba, br, bns, bd, brr, bro, brns, brd = memory.sample(total_batch_size)
    loss = 0
    for i in range(epochs):
        ...
        loss += standard_optimization_no_mem(policy_net, target_net,
                                             samples, optimizer, gamma, lambda1, lambda2, batch_size, device)
    return loss / epochs
